...
public class LimeLight extends SubsystemBase {
	 ...
	 *
	 * @param limelightSuffix When you setup the limelight you can give it a custom
	 *                        name. All setup with limelight-{name}
	 * @param knownArea       Used in combination with knownDistance to calculate
	 *                        the distance
	...
	/**
	 * 
	 * @return Aprox Distance away from the Target in View with only 2 decimal
	 *         places
	 */
	 ...
	 * <p>
	 * Place the Robot down on the ground and measure from the limelight then look
	 * at the limelight's web interface for the `ta` value. Pass the 2 values in as
	 * a constant and it will return the % of the selected KnownDistance.
	 * </p>
	 *
	 * @param precision Total Decimal Place precision.
	 * @return Approx Distance away from the Target in View with only n decimal
	 *         places
	...
	public double getShooterPower(){
		...
		if(getHeight() <= 0){
			...
			//System.out.println((-1/(1.75 * getHeight())) + 0.89);
	 ...
	 * Set the Camera Mode
	 *
	 * @param value 0, 1 0 Vision processor Driver Camera (Increases exposure,
	 ...
	 * Set Stream Mode
	 *
	 * @param value 0, 1, 2 0 Standard - Side-by-side streams if a webcam is
	 *              attached to Limelight 1 PiP Main - The secondary camera stream
	 *              is placed in the lower-right corner of the primary camera stream
	 *              2 PiP Secondary - The primary camera stream is placed in the
	...
	/**
	 * Get the Pipeline This is the true value that the limelight is using.
	 *
	 * This has an additional delay because the limelight has to read the network
	 * table value "pipeline" then set "getpipe" to the value.
	 *
	 ...
	 * Set the LED Mode
	 *
	 * @param value Mode 0, 1, 2, 3 0 Use the LED Mode set in the current pipeline 1
